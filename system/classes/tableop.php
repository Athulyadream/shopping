...
/* 		function uploadWithSave($fieldname, $filename, $path, $newfilename, $updateId)

	{
		...
		$objError	= 	$objFiles->checkFileType($filename['name'], $filename['size'], $this->arrImgType);

		if($filename['size'] > 0)

		{	

			if($objError['status'] != 1)

			{

				$this->setError($objError['msg']);
			...
			{	
				...
				$this->setError('Cannot set filename');

				return false;	

			}					

			$objError	= $objFiles->upload($filename, $path, $newThumbname, true);
			...
			//echo "$fieldname => $newThumbname";exit;

			$objError	=	$this->updateField(array($fieldname => $newThumbname),$primaryKey.'='.$updateId);
			...
			$source	=	$path.'/'.$newThumbname;	

			$dest	=	$path.'/icon_'.$newThumbname;					

			if(!copy($source,$dest))

			{	echo "Sourse : $source<br>Destination : $dest";

				exit;

			}

			$objError	=	$this->updateField(array('photo_thumb' => 'icon_'.$newThumbname),$primaryKey.'='.$updateId);	

			//$this->imageResize($source,"image");

			$this->imageResize($dest,"icon");

		}

	}

function imageResize($path,$mode)#..$path = fullpath with filename ... 
	...
	{	 
		...
		{ 
			...
			list($wnews_idth, $height, $imgType, $attr) = getimagesize($path);
			...
			{	case 'icon':					 		 			
					...
					$height	= 80 ;	$wnews_idth	= 	120;

					break;

				case 'image':				 			 			

					$height	 = 280;	$wnews_idth	=	400;
			...
			$thumb->size_wnews_idth($wnews_idth);		

			//$thumb->size_auto();			// set the biggest wnews_idth or height for thumbnail
	...
	function DeleteImage($key,$ImgField,$ImgName,$path)

	{
		...
		$objError	=	$objFiles->delFiles($path,$ImgName);

		if(is_a($objError, "PEAR_Error"))	return $objError;	

			$objError	=	$this->updateField(array($ImgField=>""),$primaryKey."=".$key);
	...
	//	$this->insert_id = mysqli_insert_id($dbconnection); $this->insert_id = mysqli_insert_id($dbconnection);

		//	echo "insertId".$this->insert_id;

		/* if (!$res)

		{

			$this->setError('An error occured :'.mysqli_error($this->connection));
...

	{
...

		{
			...
			$this->setError('An error occured argved : Invalid Arguments Supplied');

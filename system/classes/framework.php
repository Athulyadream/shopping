   ...
   public function flash($sessionName, $className){
      ...
      if(!empty($sessionName) && !empty($className) && isset($_SESSION[$sessionName])){
         ...
         $msg = $_SESSION[$sessionName];
           echo '<div class="container alert alert-'.$className.' in alert-dismissible" style="margin-top:18px;">
    <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
    <strong>'.$className.'!</strong> '.$msg.'
</div>';
         // echo "<div class=' alert ". $className ."'>".$msg."</div>";
         unset($_SESSION[$sessionName]);
